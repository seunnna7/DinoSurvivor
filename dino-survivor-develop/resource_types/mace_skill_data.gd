...
## 철퇴만의 고유 수치인 진화 형태(전방향 휘두르기 / 내려찍기)를 추가로 정의합니다.
## 넉백은 스킬 공통 필드(SkillData.knockback_distance)를 그대로 씀 — 형태별로 다르게 주고
## 싶으면(예: 내려찍기가 더 세게) 베이스/각 진화 .tres에 각자 다른 값을 적으면 됨.
